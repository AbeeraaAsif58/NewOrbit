    ...
    <section id="hero" data-reveal className="relative isolate py-4 sm:py-8">
      <div className="max-w-5xl">
        ...
        <p className="mt-5 text-lg sm:text-xl lg:text-2xl text-white/85">
          Explore curated AI projects, RAG systems, voice & vision agents. Clone, customize and
          deploy in hours.
        </p>
        ...
        <div className="mt-9 grid grid-cols-2 gap-3 sm:grid-cols-4">
          {[
            { k: "1,200+", l: "Stars/Clones" },
            { k: "85+", l: "AI Templates" },
            { k: "30%", l: "Faster Launch" },
            { k: "24/7", l: "Docs & Support" },
          ].map((it, i) => (
            <div
              key={i}
              data-reveal
...
function TrustMarquee() {
  const logos = ["OpenAI", "Hugging Face", "LangChain", "Whisper", "Supabase", "Vercel"];
  return (
  ...
  const cards = [
    {
      ...
      desc: "Embeddings + vector DB + streaming chat UI. Markdown & PDF ingestion.",
      tech: ["Next.js", "LangChain", "pgvector"],
    },
    {
      tag: "New",
      title: "Multi-Modal Vision Agent",
      desc: "Image understanding + OCR + tool-calls for reasoning.",
      tech: ["React", "OpenAI Vision", "Supabase"],
    },
    {
      tag: "Popular",
      title: "Voice Assistant Kit",
      desc: "Realtime speech (STT/TTS), wake-word & command routing.",
      tech: ["Whisper", "WebRTC", "VAD"],
    },
  ];
  return (
...
function ProjectsGrid() {
  const cards = Array.from({ length: 9 }).map((_, i) => ({
    ...
    desc:
      [
        "Product Q&A, purchase guidance, and order tracking via multi-turn chat.",
        "Indexes your help center & PDFs. Answers with citations.",
        "Transcribe, summarize, and assign tasks automatically.",
        "Generate accessible image captions with vision models.",
        "Draft replies with tone controls and dynamic placeholders.",
        "Natural language to SQL with safe schema guards.",
        "Auto-schedule posts, generate captions, suggest hashtags.",
        "Compress long docs into key points with references.",
        "Smart autofill using entities from your context.",
      ][i],
  }));
  ...
  return (
    <section id="projects" className="mt-12" data-reveal>
      <div className="mb-4 flex items-end justify-between">
        <h2 className="text-xl font-semibold sm:text-2xl">All projects</h2>
        <span className="text-sm text-white/60">{cards.length} results</span>
      </div>
    ...
    <section className="mt-12" data-reveal>
      <div className="rounded-3xl border border-white/10 bg-white/[0.06] p-6 sm:p-10 text-center shadow-[0_0_50px_-25px_rgba(255,255,255,.35)]">
        <blockquote className="mx-auto max-w-3xl text-lg text-white/90">
          “We shipped a production-ready AI support assistant in{" "}
          <span className="text-cyan-200">three days</span> using these starters. Clean
          architecture and docs made all the difference.”
        </blockquote>
        <div className="mt-3 text-sm text-white/60">— Product Lead, SaaS Co.</div>
      </div>
    </section>
  );
}
  ...
  return (
    ...
    >
      <div className="grid items-center gap-6 sm:grid-cols-[1.2fr_.8fr]">
        <div>
          <h3 className="text-2xl font-semibold">Have an AI project to list?</h3>
          <p className="mt-2 max-w-xl text-white/80">
            Submit your starter, template, or boilerplate to reach thousands of developers.
            Quality guidelines and review apply.
          </p>
        ...
        <div className="relative isolate">
          <div className="aspect-[4/3] w-full overflow-hidden rounded-2xl border border-white/10 bg-white/5">
            <div className="flex h-full items-center justify-center text-white/40">Preview</div>
          </div>
