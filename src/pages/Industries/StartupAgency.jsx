    ...
    <section id="hero" data-reveal className="relative isolate py-4 sm:py-8">
      <div className="max-w-5xl">
        <span className="inline-flex items-center gap-2 rounded-full border border-white/20 bg-white/5 px-3 py-1 text-[11px] uppercase tracking-[0.15em] text-cyan-200/80">
          <Dot /> Zero-to-One • MVP Development • Launch Strategy
        </span>

        <h1 className="mt-4 text-4xl sm:text-6xl lg:text-7xl font-semibold leading-[1.08] tracking-tight">
          Build from zero to one, <span className="text-cyan-200">launch with confidence</span> — startup acceleration
        </h1>

        <p className="mt-5 text-lg sm:text-xl lg:text-2xl text-white/85">
          Complete startup solutions from idea validation to market launch. We help you build, validate, and scale your startup.
        </p>

        <div className="mt-7 grid gap-3 sm:max-w-2xl sm:grid-cols-[1fr_auto]">
          <div className="relative">
            <input
              placeholder="Search: MVP development, product validation, go-to-market strategy…"
        ...
        <div className="mt-9 grid grid-cols-2 gap-3 sm:grid-cols-4">
          {[
            { k: "150+", l: "Startups Launched" },
            { k: "85%", l: "Success Rate" },
            { k: "$50M+", l: "Funding Raised" },
            { k: "24/7", l: "Startup Support" },
          ].map((it, i) => (
            <div
              key={i}
              data-reveal
...
function TrustMarquee() {
  const logos = ["React", "Node.js", "AWS", "Stripe", "MongoDB", "Vercel"];
  return (
  ...
  const cards = [
    {
      ...
      desc: "Rapid MVP development to validate your idea and get to market faster with minimal resources.",
      tech: ["Rapid Prototyping", "User Testing", "Iteration"],
    },
    {
      tag: "New",
      title: "Product-Market Fit",
      desc: "Strategic guidance to find product-market fit and optimize your offering for target customers.",
      tech: ["Market Research", "Customer Discovery", "Validation"],
    },
    {
      tag: "Popular",
      title: "Go-to-Market Strategy",
      desc: "Complete go-to-market strategy including positioning, pricing, and launch execution.",
      tech: ["Strategy", "Positioning", "Launch Planning"],
    },
  ];
  return (
...
function ProjectsGrid() {
  const cards = Array.from({ length: 9 }).map((_, i) => ({
    ...
    desc:
      [
        "Launched a fintech MVP that raised $2M in seed funding and acquired 10K users in 6 months.",
        "Validated a SaaS product idea and achieved product-market fit with 500+ paying customers.",
        "Built an e-commerce platform that processed $1M in transactions within the first year.",
        "Developed a healthtech solution that improved patient outcomes by 40% and raised Series A.",
        "Created an edtech platform serving 50K+ students with personalized learning experiences.",
        "Launched a proptech solution that streamlined real estate transactions and raised $5M.",
        "Built a foodtech marketplace connecting local restaurants with customers, achieving profitability.",
        "Developed a cleantech innovation that reduced energy consumption by 30% in target markets.",
        "Created a social impact app that helped 100K+ users and secured $1M in impact funding.",
      ][i],
  }));
  ...
  return (
    <section id="projects" className="mt-12" data-reveal>
      <div className="mb-4 flex items-end justify-between">
        <h2 className="text-xl font-semibold sm:text-2xl">Startup success stories</h2>
    ...
    <section className="mt-12" data-reveal>
      <div className="rounded-3xl border border-white/10 bg-white/[0.06] p-6 sm:p-10 text-center shadow-[0_0_50px_-25px_rgba(255,255,255,.35)]">
        <blockquote className="mx-auto max-w-3xl text-lg text-white/90">
          "They helped us go from idea to{" "}
          <span className="text-cyan-200">$2M in funding</span> in just 8 months. The startup expertise was invaluable."
        </blockquote>
        <div className="mt-3 text-sm text-white/60">— Founder, TechStart Inc.</div>
      </div>
    </section>
  );
}
  ...
  return (
    ...
    >
      <div className="grid items-center gap-6 sm:grid-cols-[1.2fr_.8fr]">
        <div>
          <h3 className="text-2xl font-semibold">Ready to launch your startup?</h3>
          <p className="mt-2 max-w-xl text-white/80">
            Let's turn your idea into a successful startup with our proven methodology and expert guidance.
          </p>
        ...
        <div className="relative isolate">
          <div className="aspect-[4/3] w-full overflow-hidden rounded-2xl border border-white/10 bg-white/5">
            <div className="flex h-full items-center justify-center text-white/40">Startup Dashboard</div>
          </div>
  ...
  return (
    <article className="group relative overflow-hidden rounded-2xl border border-white/15 bg-white/10 p-5 transition hover:-translate-y-1.5 hover:border-cyan-200/50">
      <div className="aspect-video w-full overflow-hidden rounded-xl border border-white/10 bg-white/5">
        <div className="flex h-full items-center justify-center text-white/40">Startup Preview</div>
      </div>
      <h3 className="mt-3 text-lg font-semibold transition-colors group-hover:text-cyan-200">{title}</h3>
      ...
      <div className="mt-3 flex items-center justify-between">
        <div className="flex items-center gap-3 text-xs text-white/70">
          ...
          <span>•</span>
          <span>${(1 + idx * 0.5).toFixed(1)}M raised</span>
        </div>
        <button className="rounded-xl bg-cyan-300/90 px-3.5 py-2 text-sm font-medium text-[#0E1730] hover:bg-cyan-300">
          View case study
        </button>
