  ...
  return (
    ...
    <div className="px-4 sm:px-6 md:px-[8vh] image-container grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-6 md:gap-[2vh]">
      {cardsData.slice(0, visibleCards).map((card) => (
        <div
          key={card.id}
          className="h-[40vh] sm:h-[45vh] md:h-[50vh] w-full sm:w-auto rounded-lg sm:rounded-xl md:rounded-[10px] relative overflow-hidden group"
        >
          <img
            className="h-full w-full rounded-lg sm:rounded-xl md:rounded-[10px] object-cover"
            src={card.image}
          />
          <div
            className="absolute inset-0 bg-gradient-to-b to-[#0006] from-transparent opacity-0 group-hover:opacity-100 group-hover:to-transparent group-hover:from-[#0006]"
          ></div>
