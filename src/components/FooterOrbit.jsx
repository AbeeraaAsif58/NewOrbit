...
export default function FooterOrbitMoonTech() {
  ...
  useEffect(() => {
    const setupAnimations = () => {
      ...
      // Only run animations on desktop (screen width > 1024px)
      if (isMobileOrTablet) {
        // On mobile/tablet, just set initial styles without animations
        if (root.current) {
          root.current.style.opacity = '1';
          root.current.style.transform = 'translateY(0)';
          
          // Set all stagger elements to visible
          const staggerElements = root.current.querySelectorAll('.ft-stagger');
  ...
  return (
    <>
      <style jsx>{`
        /* Ensure footer elements are visible on mobile/tablet without animations */
        @media (max-width: 1024px) {
          .footer-mobile-visible {
            opacity: 1 !important;
            transform: translateY(0) !important;
          }
          .footer-mobile-visible .ft-stagger {
            opacity: 1 !important;
            transform: translateY(0) !important;
          }
        }
      ...
      <footer
        ref={root}
        className="relative z-[100] w-full text-white mb-0 block footer-mobile-visible"
        style={{
          background:
