...
function Marquee() {
 let marqueeElem = Array.from({length:4}).map((ele,index)=>(
  <motion.h1 initial={{x:0}} animate={{x:"-100%"}} transition={{ease:"linear", repeat:Infinity, duration:10}} key={index} className='text-[25vw] sm:text-[22vw] md:text-[20vw] font-bold font-title -mb-4 sm:-mb-5 md:-mb-6 pr-8 sm:pr-10'>We Are Ochi</motion.h1>
 ))
  return (
